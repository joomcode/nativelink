...
The `store_eviction_reason` label has two values. `capacity` means that
capacity or timeout pressure dropped the item. This is churn. `replaced` means
that an insert overwrote the same key, for example a re-upload of the same
digest. Filter churn queries on `store_eviction_reason="capacity"`. Without
that filter a busy CI looks like a thrashing cache.
...
Share of a store that capacity pressure replaces per second:
...
Evicted bytes per inserted byte. Above 1 the store cannot hold the working set:

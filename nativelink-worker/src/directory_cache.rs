...
impl DirectoryCache {
    ...
    async fn hardlink_executable_variant(
        ...
        // be materialized from an on-disk 0o444 blob. This may fetch from the
        // slow store, so it holds a fetch permit — same as `hardlink_cas_blob`,
        // otherwise executable inputs would fetch outside the cache-wide
        // `max_concurrent_fetches` budget.
        {
            let _permit = self.acquire_fetch_permit().await?;
            self.cas_store
                .populate_fast_store(StoreKey::Digest(*digest))
